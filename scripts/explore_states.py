...
if __name__ == "__main__": # for multiprocessing
    ...
    parser = argparse.ArgumentParser(description='Explore the states visited during JOAT training. Not used to train actual model')

    parser.add_argument('--json_path', action='store', help='Path to where you want the json of visited states.')

    parser.add_argument('--loading_path', action='store', default=None, help='Path to the learning model weights. Should not be set to explore state spaces from scratch!')
    ...
    parser.add_argument('--workers', action='store', default='2', type=int, help='The number of workers to use to process self- and arena-play. A value >1 will leverage multiprocessing. Non-distributed version of Explorer not available (default: 2)')

    parser.add_argument('--games', dest='games', action='store', nargs='+', default=['gardner', 'mallet', 'baby', 'rifle', 'atomic'], choices=['gardner', 'mallet', 'baby', 'rifle', 'atomic'], type=str, help='The games to consider during training. (default: all variants)')
    ...
    train_args = dict({
        ...
        'json_folder_file': ('/'.join(args.json_path.split('/')[:-1]),args.json_path.split('/')[-1]) if not (args.json_path is None) else '',
    })
    ...
    # handle imports
    if args.workers > 1:
        from learning.alpha_zero.distributed.explorer import Explorer
        ...
        log.warning('num_workers < 2 invalid: Non-distributed version of Explorer not implemented.')
        raise ValueError('num_workers < 2 invalid: Non-distributed version of Explorer not implemented.')
        from learning.alpha_zero.undistributed.explorer import Explorer
    ...
    log.info('Loading the JOAT Explorer...')

        
    e = Explorer(games, probs, nnet, train_args)
